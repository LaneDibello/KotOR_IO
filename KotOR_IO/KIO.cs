...
namespace KotOR_IO 
{
    ...
    public static class KReader 
    {
        ...
        static public BIF ReadBIF(Stream s)
        {
            ...
            for (int i = 0; i < b.Variable_Resource_Count; i++)
            {
                ...
                catch (KeyNotFoundException) { VRE.Resource_Type_code = 0; VRE.Resource_Type_text = "null"; }
                VRE.IDx = VRE.ID >> 20;
    ...
    public static class kWriter
    {
        ...
        /// <param name="l"></param>
        /// <param name="s"></param>
        public static void Write(LTR l, Stream s)
        {
            BinaryWriter bw = new BinaryWriter(s);

            bw.Write(l.FileType.ToArray());
            bw.Write(l.Version.ToArray());

            bw.Write((byte)28);

            //singles
            foreach (float f in l.data.singles.start)
            {
                bw.Write(f);
            }
            foreach (float f in l.data.singles.middle)
            {
                bw.Write(f);
            }
            foreach (float f in l.data.singles.end)
            {
                bw.Write(f);
            }

            //Doubles
            foreach (LTR.cdf c in l.data.doubles)
            {
            ...
            //triples
            foreach (LTR.cdf[] ca in l.data.triples)
            {
